...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        if self.cuda:
            ...
            if self.cfg.common.hmp:
                self.cfg.common.hpu_mixed_precision_mode = 'hmp'
            ...
            if self.cfg.common.hpu_mixed_precision_mode == 'hmp':
               self.use_hmp = True
               dir_path = os.path.dirname(os.path.realpath(__file__))
               bf16_file_path = os.path.join(dir_path, self.cfg.common.hmp_bf16)
               fp32_file_path = os.path.join(dir_path, self.cfg.common.hmp_fp32)
               from habana_frameworks.torch.hpex import hmp
               hmp.convert(opt_level=self.cfg.common.hmp_opt_level,
                           bf16_file_path=bf16_file_path,
                           fp32_file_path=fp32_file_path,
                           isVerbose=self.cfg.common.hmp_verbose)
            elif self.cfg.common.hpu_mixed_precision_mode == 'autocast':
                self.use_autocast_on_gaudi=True
            assert not (self.use_hmp and self.use_autocast_on_gaudi)
        ...
        if self.cfg.common.hmp:
            assert self.cfg.common.hpu_mixed_precision_mode == 'hmp'
            assert self.use_hmp == True
            assert not (self.use_autocast_on_gaudi and self.use_hmp)
